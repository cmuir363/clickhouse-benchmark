...
WITH factories AS (
    SELECT factoryId
    FROM iot_analytics.iot_metadata
    ORDER BY rand()
    LIMIT 3
)
SELECT
...
all_values_for_specific_sensors_results = client.query(get_all_values_for_specific_sensors_query)
print(f"Values for specific sensors query id: {all_values_for_specific_sensors_results.query_id}")
...
total_sensor_count_for_owner_results = client.query(get_total_sensor_count_for_owner_query)
print(f"Total sensor count for owner query id: {total_sensor_count_for_owner_results.query_id}")
...
values_beyond_95th_percentile_for_one_owner_results = client.query(get_values_beyond_95th_percentile_for_one_owner_query)
print(f"Values beyond 95th percentile for one owner query id: {values_beyond_95th_percentile_for_one_owner_results.query_id}")
...
percentile_value_per_sensor_type_per_factory_results = client.query(get_90th_percentile_value_per_sensor_type_per_factory_query)
print(f"90th percentile value per sensor type per factory query id: {percentile_value_per_sensor_type_per_factory_results.query_id}")
...
all_temperature_values_per_factory_results = client.query(get_all_temperature_values_per_factory_query)
print(f"All temperature values per factory query id: {all_temperature_values_per_factory_results.query_id}")
...
num_values_per_owner_per_factory_results = client.query(get_num_values_per_owner_per_factory_query)
print(f"Number of values per owner per factory query id: {num_values_per_owner_per_factory_results.query_id}")
